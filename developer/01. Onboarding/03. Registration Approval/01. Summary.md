...
To fetch the specific checklist process step details; status; possible available comment as well as the retriggerable status, the following endpoint need to get called
<br>
...
<br>
        ...
        Does the specific ChecklistItem have a Retrigger ProcessStep in todo (e.g. RETRIGGER_BUSINESS_PARTNER_NUMBER_PUSH) ? - if yes retrigger identifier will be true, if no false    
